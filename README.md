...
1. 识字界面
  - 页首展示今日可初识的字数限额和当前进度，如`5/30`，可用环形进度条展示
...
2. 测验界面
  - 页首参考 `views/MyCCardsView.vue` 通过PageHeader展示今日可掌握的字数限额和当前进度，如`3/30`，可用环形进度条展示，右上角展示魔力值
  - 测验类型为选择题：随机选取一个`已初识`的字作为目标字，从其全部例句中随机选取1个，并将词语中的目标字替换成`（__）`，再随机从所有其它字中选取3个，和目标字一起组成选择题的4个选项，打乱顺序并展示，初始展示时，要高亮题目并读一遍题目，并将其中的`（__）`读作`什么的`，遇到`（__）`要停顿5秒后再读
  - 点击答案后，会展示正确或错误的动画（如对号、叉号等），如果错误需要展示正确答案。
  - 如果正确，该字测验正确次数+1；如果该字测验正确总共达到3次，则记该字为“已掌握”，不再出现该字相关的测试题，并奖励魔力值`3`点，今日可掌握的字数`-1`，并在延迟2s后自动进入下一题
  - 如果错误，该字测验错误次数+1，同时弹窗展示该字的识字卡片，并在识字卡片阅读完成后显示`下一题`按钮，点击后进入下一题
  - 下一题可以是今日可掌握的其它任何字，但除非只剩下一个字可测验，否则不能和上一题的字相同
  - 今日测验全部结束后，会展示今日测验的正确率，如`正确率：80%`，展示庆祝动画（如烟花、气球等）和“前往兑换画片”按钮，点击后进入`画片界面`
3. 已掌握汉字统计界面
